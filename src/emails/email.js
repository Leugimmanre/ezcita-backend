...
export const sendAppointmentEmail = async ({
  ...
  if (needsICS) {
    ...
    // Usar los offsets de la cita como alarmas del calendario
    const alarmsMinutes =
      Array.isArray(appointment?.reminders?.emailOffsets) &&
      appointment.reminders.emailOffsets.length
        ? appointment.reminders.emailOffsets
        : [1440, 60]; // fallback opcional
